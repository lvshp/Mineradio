...
function renderMiniQueuePanel(opts) {
  ...
  $list.innerHTML = playQueue.map(function(song, i){
    ...
    return '<div class="mini-queue-item' + (i === currentIdx ? ' now' : '') + '" data-action="playQueueAt" data-action-value="' + i + '">' +
      imgTag +
      ...
      '<button class="mini-queue-remove mini-queue-next" data-stop-propagation="true" data-action="queueIndexNext" data-action-value="' + i + '" title="下一首播放">下</button>' +
      '<button class="mini-queue-remove" data-stop-propagation="true" data-action="removeFromQueue" data-action-value="' + i + '" title="移除">×</button>' +
    '</div>';
  }).join('');
...
function renderQueuePanel(opts) {
  ...
  $ql.innerHTML = playQueue.map(function(song, i){
    ...
    return '<div class="queue-item' + (i === currentIdx ? ' now' : '') + '" data-action="playQueueAt" data-action-value="' + i + '">' +
      imgTag +
      '<div class="qi-info"><div class="qi-name">' + escHtml(song.name) + '</div><div class="qi-sub"><button class="queue-artist-link" type="button" data-stop-propagation="true" data-action="openQueueArtist" data-action-value="' + i + '">' + escHtml(song.artist || '未知歌手') + '</button></div></div>' +
      '<div class="qi-act">' +
        '<button class="' + (isSongLiked(song) ? 'liked' : '') + '" data-stop-propagation="true" data-action="toggleLikeQueueIndex" data-action-value="' + i + '" title="' + (isSongLiked(song) ? '取消红心' : '红心喜欢') + '">' + heartIconSvg() + '</button>' +
        '<button class="queue-next" data-stop-propagation="true" data-action="queueIndexNext" data-action-value="' + i + '" title="下一首播放">下</button>' +
        '<button data-stop-propagation="true" data-action="collectQueueIndex" data-action-value="' + i + '" title="收藏到歌单">' + playlistPlusIconSvg() + '</button>' +
        '<button data-stop-propagation="true" data-action="removeFromQueue" data-action-value="' + i + '" title="移除">×</button>' +
      '</div>' +
    '</div>';
  }).join('');
...
function playlistPanelProviderId(provider, id) {
  if (provider === 'navidrome') return 'navidrome:' + id;
...
function playlistPanelDetailHtml(pl, provider) {
  ...
  var cover = pl && pl.cover ? (provider === 'qq' || provider === 'navidrome' ? pl.cover : (pl.cover + '?param=96y96')) : '';
  ...
  return '<div class="pl-inline-detail" data-pl-detail="' + escHtml(key) + '">' +
    '<div class="pl-detail-sticky">' +
      '<div class="pl-detail-head">' + img + '<div style="flex:1;min-width:0"><div class="pl-detail-title">' + escHtml(pl.name || '歌单详情') + '</div><div class="pl-detail-sub">' + escHtml((pl.trackCount || tracks.length || 0) + ' 首 · ' + (pl.creator || (provider === 'navidrome' ? 'Navidrome' : (provider === 'qq' ? 'QQ 音乐' : '网易云音乐')))) + '</div></div><div class="pl-detail-count">' + (loading ? '载入中' : (renderLimit + '/' + tracks.length)) + '</div></div>' +
...
async function openPlaylistPanelDetail(provider, pid, title) {
  ...
  try {
    var r = provider === 'navidrome'
      ? await apiJson('/api/navidrome/playlist/tracks?id=' + encodeURIComponent(pid))
      : (provider === 'qq'
...
function playPlaylistPanelDetail() {
  ...
  var provider = parts[0] === 'navidrome' ? 'navidrome' : (parts[0] === 'qq' ? 'qq' : 'netease');
...
async function loadPlaylistIntoQueueById(id, autoplay, title) {
  ...
  var navidromePlaylistId = String(id || '').indexOf('navidrome:') === 0 ? String(id).slice(10) : '';
  var r = null;
  try {
    r = navidromePlaylistId
  ...
  try {
    ...
    if (!qqPlaylistId && !navidromePlaylistId && isLikedPlaylistContext(id, title, r.playlist)) markSongsLiked(playQueue, true);
    if (!qqPlaylistId && !navidromePlaylistId) syncLikeStatusForSongs(playQueue);
    currentIdx = 0;
