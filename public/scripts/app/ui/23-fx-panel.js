...
function renderUserFxArchives() {
  ...
  grid.innerHTML = userFxArchives.map(function(slot, index){
    ...
    var nameHtml = editing
      ? '<input class="user-archive-input" id="user-archive-input-' + index + '" type="text" maxlength="18" value="' + escHtml(slot.name) + '" data-action-keydown="handleUserFxArchiveRenameKey" data-action-value="' + index + '">'
      : '<div class="user-archive-name" title="' + escHtml(slot.name) + '">' + escHtml(slot.name) + '</div>';
    var actionsHtml = editing
      ? '<button type="button" data-action="commitUserFxArchiveRename" data-action-value="' + index + '">确定</button>' +
        '<button type="button" data-action="cancelUserFxArchiveRename">取消</button>'
      : '<button type="button" data-action="applyUserFxArchive" data-action-value="' + index + '"' + (hasSave ? '' : ' disabled') + '>应用</button>' +
        '<button type="button" data-action="saveUserFxArchive" data-action-value="' + index + '">保存</button>' +
        '<button type="button" data-action="renameUserFxArchive" data-action-value="' + index + '">命名</button>';
...
function renderUserFxArchives() {
  ...
  var cards = userFxArchives.map(function(slot, index){
    ...
    var editing = userFxArchiveEditing === index;
    var nameHtml = editing
      ? '<input class="user-archive-input" id="user-archive-input-' + index + '" type="text" maxlength="28" value="' + escHtml(slot.name) + '" data-action-keydown="handleUserFxArchiveRenameKey" data-action-value="' + index + '">'
      : '<div class="user-archive-name" title="' + escHtml(slot.name) + '">' + escHtml(slot.name) + '</div>';
    var actionsHtml = editing
      ? '<button type="button" data-action="commitUserFxArchiveRename" data-action-value="' + index + '">确定</button>' +
        '<button type="button" data-action="cancelUserFxArchiveRename">取消</button>'
      : '<button type="button" data-action="applyUserFxArchive" data-action-value="' + index + '"' + (hasSave ? '' : ' disabled') + '>应用</button>' +
        '<button type="button" data-action="saveUserFxArchive" data-action-value="' + index + '">保存</button>' +
        '<button type="button" data-action="renameUserFxArchive" data-action-value="' + index + '">命名</button>' +
        '<button type="button" data-action="exportUserFxArchive" data-action-value="' + index + '"' + (hasSave ? '' : ' disabled') + '>导出</button>' +
        '<button type="button" data-action="removeUserFxArchive" data-action-value="' + index + '">删除</button>';
    return '<div class="user-archive-slot' + (hasSave ? ' has-save' : '') + '" data-slot="' + index + '">' +
      nameHtml +
...
function buildLyricColorControls() {
  ...
  html += lyricColorPresets.map(function(p, i){
    return '<button class="lyric-swatch" type="button" data-color="' + p.color + '" data-action="setLyricColorPreset" data-action-value="' + i + '" title="' + escHtml(p.name) + '" style="--swatch:' + p.color + '"></button>';
  }).join('');
  grid.innerHTML = html;
}
...
function renderCoverPickerSwatches() {
  ...
  wrap.innerHTML = colors.map(function(c){
    return '<button type="button" style="--c:' + c + '" title="' + c.toUpperCase() + '" data-action="applyCoverPickerColor" data-action-value="' + c + '"></button>';
  }).join('');
}
...
function buildPresetGrid() {
  ...
  grid.innerHTML = order.map(function(i){
    ...
    return '<div class="preset-card" data-preset="' + i + '" data-action="setPreset" data-action-value="' + i + '">' +
