...
function platformMeta(provider) {
  if (provider === 'navidrome') return { key: 'navidrome', short: 'ND', label: 'Navidrome', app: 'Navidrome', dot: 'navidrome' };
...
function platformStatus(provider) {
  if (provider === 'navidrome') return navidromeStatus;
...
function hasAnyPlatformLogin() {
  return hasPlatformLogin('netease') || hasPlatformLogin('qq') || hasPlatformLogin('navidrome');
}
function firstLoggedProvider() {
  ...
  if (hasPlatformLogin('navidrome')) return 'navidrome';
  return 'netease';
}
function providerAvatarSrc(provider, status) {
  ...
  var fill = provider === 'navidrome' ? '#5abedc' : (provider === 'qq' ? '#bfd66b' : '#d95b67');
  var bg = provider === 'navidrome' ? '#07151b' : (provider === 'qq' ? '#11150b' : '#180b0f');
...
function normalizeNavidromeStatus(info) {
  return Object.assign({
    ...
    provider: 'navidrome',
    loggedIn: !!(info && (info.loggedIn || info.configured)),
    configured: !!(info && info.configured),
    nickname: (info && (info.nickname || info.username)) || 'Navidrome',
  });
}
async function refreshNavidromeStatus() {
  try {
    navidromeStatus = normalizeNavidromeStatus(await apiJson('/api/navidrome/status?t=' + Date.now()));
    if (navidromeStatus.loggedIn && !hasPlatformLogin(activeAccountProvider)) activeAccountProvider = 'navidrome';
    renderUserBtn();
    return navidromeStatus;
  } catch (e) {
    console.warn('Navidrome status failed:', e);
...
function renderUserBtn() {
  ...
  if (dualAccountMode && hasAnyPlatformLogin()) {
    ...
    btn.innerHTML = renderTopAccountPill('netease') + renderTopAccountPill('qq') + renderTopAccountPill('navidrome');
...
async function showLoginModal(opts) {
  opts = opts || {};
  if (opts.provider) loginProvider = opts.provider === 'qq' || opts.provider === 'navidrome' ? opts.provider : 'netease';
...
function setLoginProvider(provider, silent) {
  loginProvider = provider === 'qq' || provider === 'navidrome' ? provider : 'netease';
  updateLoginProviderUi();
...
function updateLoginProviderUi() {
  ...
  var isNavidrome = loginProvider === 'navidrome';
  ...
  var navidromePanel = document.getElementById('navidrome-config-panel');
  ...
  var navidromeBtn = document.getElementById('login-provider-navidrome');
  ...
  if (navidromeBtn) navidromeBtn.classList.toggle('active', isNavidrome);
  if (title) title.textContent = '扫码登录' + meta.label;
  if (title && isNavidrome) title.textContent = '连接 Navidrome';
  if (desc) desc.innerHTML = isNavidrome
    ? '连接你的 <b>Navidrome / Subsonic</b> 服务后，可搜索并播放私人曲库。'
    : (isQQ
  ...
  if (shell) {
    shell.classList.toggle('web-login-preview', isQQ || isNavidrome || canOpenNeteaseWeb);
    shell.classList.toggle('qq-preview', isQQ);
    shell.classList.toggle('netease-preview', !isQQ && !isNavidrome && canOpenNeteaseWeb);
    shell.style.display = isNavidrome ? 'none' : '';
  }
  if (qqPanel) qqPanel.classList.toggle('show', isQQ && qqManualCookieOpen);
  if (navidromePanel) navidromePanel.classList.toggle('show', isNavidrome);
  if (qqCookieToggle) {
  ...
  if (st) {
    st.className = (isQQ || isNavidrome) ? 'preview' : '';
    st.textContent = isNavidrome
      ? (navidromeStatus.configured ? ('已连接 Navidrome · ' + (navidromeStatus.username || navidromeStatus.serverUrl || '')) : '填写 Navidrome 地址和账号后连接')
      : (isQQ
  ...
  if (refreshBtn) {
    refreshBtn.disabled = isNavidrome ? !!navidromeConfigBusy : (isQQ ? !!qqWebLoginBusy : !!neteaseWebLoginBusy);
    refreshBtn.textContent = isNavidrome ? (navidromeStatus.configured ? '重新检测' : '连接') : (isQQ ? (qqWebLoginBusy ? '等待扫码…' : '扫码登录') : (canOpenNeteaseWeb ? (neteaseWebLoginBusy ? '等待扫码…' : '网页登录') : '刷新二维码'));
    refreshBtn.setAttribute('data-action', isNavidrome ? 'submitNavidromeConfig' : 'loginRefresh');
  }
}
async function refreshQr() {
  stopQrPoll();
  updateLoginProviderUi();
  if (loginProvider === 'navidrome') {
    var ndStatus = document.getElementById('qr-status');
    var nd = await refreshNavidromeStatus();
    var urlInput = document.getElementById('navidrome-url-input');
    var userInput = document.getElementById('navidrome-user-input');
    if (urlInput && nd.serverUrl) urlInput.value = nd.serverUrl;
    if (userInput && nd.username) userInput.value = nd.username;
    if (ndStatus) {
      ndStatus.textContent = nd.configured ? ('已连接 Navidrome · ' + (nd.username || nd.serverUrl || '')) : '填写 Navidrome 地址和账号后连接';
      ndStatus.className = 'preview';
    }
    return;
  }
  if (loginProvider === 'qq') {
...
async function submitNavidromeConfig() {
  if (navidromeConfigBusy) return;
  var statusEl = document.getElementById('qr-status');
  var saveBtn = document.getElementById('navidrome-save-btn');
  var serverUrl = document.getElementById('navidrome-url-input');
  var username = document.getElementById('navidrome-user-input');
  var password = document.getElementById('navidrome-password-input');
  var body = {
  ...
  if (!body.serverUrl || !body.username || !body.password) {
    if (statusEl) { statusEl.textContent = '请填写服务器地址、用户名和密码'; statusEl.className = 'fail'; }
    return;
  }
  navidromeConfigBusy = true;
  if (saveBtn) saveBtn.classList.add('busy');
  if (statusEl) { statusEl.textContent = '正在连接 Navidrome…'; statusEl.className = 'preview'; }
  try {
    var info = await apiJson('/api/navidrome/config', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(body)
    });
    if (!info || !info.configured) throw new Error((info && (info.message || info.error)) || 'Navidrome 连接失败');
    navidromeStatus = normalizeNavidromeStatus(info);
    activeAccountProvider = 'navidrome';
    if (password) password.value = '';
    renderUserBtn();
    refreshUserPlaylists(true);
    loadHomeDiscover(true);
    if (statusEl) { statusEl.textContent = 'Navidrome 已连接'; statusEl.className = 'scan'; }
    setTimeout(function(){
      closeLoginModal();
      showToast('Navidrome 已连接: ' + (info.username || info.serverUrl || ''));
    }, 420);
  } catch (e) {
    if (statusEl) { statusEl.textContent = e && e.message ? e.message : 'Navidrome 连接失败'; statusEl.className = 'fail'; }
  } finally {
    navidromeConfigBusy = false;
    if (saveBtn) saveBtn.classList.remove('busy');
    updateLoginProviderUi();
  }
}
...
function updateUserModalUi() {
  ...
  var addNavidrome = document.getElementById('account-add-navidrome');
  if (chip) {
  ...
  if (vipEl) {
    ...
    } else if (activeAccountProvider === 'navidrome') {
      vipEl.textContent = (st && st.serverUrl) ? st.serverUrl : 'Subsonic API 已连接';
      vipEl.style.color = 'rgba(90,190,220,0.72)';
    } else {
  ...
  ['netease','qq','navidrome','both'].forEach(function(key){
  ...
  if (addNavidrome) addNavidrome.textContent = hasPlatformLogin('navidrome') ? '查看 Navidrome' : '连接 Navidrome';
  if (logoutBtn) logoutBtn.textContent = activeAccountProvider === 'navidrome' ? '断开 Navidrome' : (activeAccountProvider === 'qq' ? '退出 QQ 音乐' : '退出网易云');
