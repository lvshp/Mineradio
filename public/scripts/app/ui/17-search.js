...
function updateSearchModeTabs() {
  ...
  var navidromeBtn = document.getElementById('search-mode-navidrome');
  ...
  if (navidromeBtn) {
    navidromeBtn.classList.toggle('active', searchMode === 'navidrome');
    navidromeBtn.setAttribute('aria-selected', searchMode === 'navidrome' ? 'true' : 'false');
  }
  if (podcastBtn) {
  ...
  if ($input) {
    $input.placeholder = searchMode === 'podcast'
      ? '搜索播客、电台...'
      : (searchMode === 'navidrome' ? '搜索 Navidrome...' : (searchMode === 'qq' ? '搜索 QQ 音乐...' : (searchMode === 'netease' ? '搜索网易云音乐...' : '搜索歌曲、歌手...')));
  }
  requestAnimationFrame(updateSearchPillGlassDisplacementMap);
}
function setSearchMode(mode) {
  mode = (mode === 'podcast' || mode === 'netease' || mode === 'qq' || mode === 'navidrome') ? mode : 'song';
...
function renderPodcastRadios(items, label) {
  ...
  $results.innerHTML = podcastResults.map(function(p, i){
    return '<div class="search-result">' +
      '<div style="display:flex;align-items:center;gap:12px;flex:1;min-width:0" data-action="openPodcastPrograms" data-action-value="' + i + '">' +
        ...
        '</div>' +
      '</div>' +
      '<button class="add-btn" title="Open" data-stop-propagation="true" data-action="openPodcastPrograms" data-action-value="' + i + '">›</button>' +
    '</div>';
  }).join('');
  $results.classList.add('show');
...
function renderPodcastPrograms() {
  ...
  if (!podcastPrograms.length) {
    $results.innerHTML = '<div class="podcast-result-head"><button class="podcast-back-btn" data-stop-propagation="true" data-action="renderPodcastRadios">‹</button><div class="search-result-info"><div class="search-result-title">' + escHtml(radio.name || 'Podcast') + '</div><div class="search-result-meta">No playable episodes</div></div></div>';
    $results.classList.add('show');
    return;
  }
  $results.innerHTML =
    '<div class="podcast-result-head">' +
      '<button class="podcast-back-btn" data-stop-propagation="true" data-action="renderPodcastRadios">‹</button>' +
    ...
    podcastPrograms.map(function(p, i){
      return '<div class="search-result">' +
        '<div style="display:flex;align-items:center;gap:12px;flex:1;min-width:0" data-action="playPodcastProgram" data-action-value="' + i + '">' +
          searchThumbHtml(p.cover) +
          '<div class="search-result-info">' +
          ...
          '</div>' +
        '</div>' +
        '<button class="add-btn" title="下一首播放" data-stop-propagation="true" data-action="queuePodcastProgram" data-action-value="' + i + '">+</button>' +
      '</div>';
    }).join('');
  $results.classList.add('show');
...
function songProviderKey(song) {
  if (song && (song.provider === 'navidrome' || song.source === 'navidrome' || song.type === 'navidrome')) return 'navidrome';
...
function songSourceTagHtml(song) {
  ...
  var label = key === 'navidrome' ? 'ND' : (key === 'qq' ? 'QQ' : 'NE');
...
function searchResultMetaText(song) {
  ...
  if (songProviderKey(song) === 'navidrome' && song.playable === false) bits.push('Navidrome 暂不可播');
...
function searchResultMetaHtml(song, index) {
  ...
  return '<button class="search-artist-link" type="button" data-stop-propagation="true" data-action="openSearchResultArtist" data-action-value="' + index + '">' + escHtml(artist) + '</button>' + tail;
}
...
function scoreSongSearchResult(song, q, sourceIndex) {
  ...
  if (songProviderKey(song) === 'navidrome') score += 10;
...
function mergeSongSearchResults(neteaseSongs, qqSongs, limit, q, navidromeSongs) {
  var out = [];
  var seen = {};
  ...
  (navidromeSongs || []).forEach(function(song, i){ push(song, i); });
...
async function fetchMusicSearchResults(q, mode) {
  ...
  if (mode === 'navidrome') {
    var ndOnly = await apiJson('/api/navidrome/search?keywords=' + encodeURIComponent(q) + '&limit=18');
    if (!ndOnly.configured && (!ndOnly.songs || !ndOnly.songs.length)) {
      showToast('先在登录面板连接 Navidrome');
      openProviderLogin('navidrome');
    }
    return mergeSongSearchResults([], [], 18, q, ndOnly.songs || []);
  }
  if (mode === 'netease') {
  ...
  if (navidromeStatus && navidromeStatus.configured) tasks.push(apiJson('/api/navidrome/search?keywords=' + encodeURIComponent(q) + '&limit=12'));
  ...
  var navidromeSongs = result[2] && result[2].status === 'fulfilled' ? ((result[2].value && result[2].value.songs) || []) : [];
  if (result[1].status === 'rejected') console.warn('QQ search failed:', result[1].reason);
  if (result[2] && result[2].status === 'rejected') console.warn('Navidrome search failed:', result[2].reason);
  return mergeSongSearchResults(neteaseSongs, qqSongs, 18, q, navidromeSongs);
}
function renderSongSearchResults(songs) {
  ...
  $results.innerHTML = playlist.map(function(s, i){
    ...
    return '<div class="search-result ' + sourceClass + '">' +
      '<div style="display:flex;align-items:center;gap:12px;flex:1;min-width:0" data-action="playSearchResult" data-action-value="' + i + '">' +
        imgTag +
        '<div class="search-result-info">' +
        ...
        '</div>' +
      '</div>' +
      '<button class="song-action-btn' + (isSongLiked(s) ? ' liked' : '') + '" data-like-index="' + i + '" title="' + (isSongLiked(s) ? '取消红心' : '红心喜欢') + '" data-stop-propagation="true" data-action="toggleLikeSearchResult" data-action-value="' + i + '">' + heartIconSvg() + '</button>' +
      '<button class="song-action-btn" title="收藏到歌单" data-stop-propagation="true" data-action="collectSearchResult" data-action-value="' + i + '">' + playlistPlusIconSvg() + '</button>' +
      '<button class="add-btn" title="下一首播放" data-stop-propagation="true" data-action="queueSearchResult" data-action-value="' + i + '">+</button>' +
    '</div>';
  }).join('');
  $results.classList.add('show');
