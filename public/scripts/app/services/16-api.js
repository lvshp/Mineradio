...
function isSameOriginCoverUrl(url) {
  return /^\/api\/navidrome\/(?:cover|song\/cover)(?:\?|$)/i.test(String(url || ''));
}
function coverProxySrc(url, cacheBust) {
  if (!url) return '';
  if (isInlineCoverSrc(url)) return url;
  if (isSameOriginCoverUrl(url)) return url + (cacheBust ? (url.indexOf('?') >= 0 ? '&' : '?') + 'v=' + Date.now() : '');
...
function songCustomCoverKey(song) {
  ...
  if (song.provider === 'navidrome' || song.source === 'navidrome' || song.type === 'navidrome') return 'navidrome:' + (song.id || (song.name + '|' + song.artist));
...
function renderHomeTiles() {
  ...
  row.innerHTML = tiles.map(function(item, i){
    ...
    return '<button class="home-tile' + (!cover && homeDiscoverState.loading ? ' home-skeleton' : '') + '" data-home-tone="' + escHtml(tone) + '" type="button" data-action="handleHomeTileClick" data-action-value="' + i + '">' +
...
function songSourceLabel(song) {
  if (!song) return '未知';
  if (song.provider === 'navidrome' || song.source === 'navidrome' || song.type === 'navidrome') return 'Navidrome';
...
function currentArtistId(song) {
  if (!song) return '';
  if (!isCloudSong(song) && songProviderKey(song) !== 'navidrome') return '';
...
function renderArtistSongList(songs) {
  ...
  return '<div class="detail-scroll">' + detailArtistSongs.map(function(s, i){
    ...
    var actionsHtml = '<div class="artist-song-actions">' +
      '<button class="artist-song-action collect" type="button" title="收藏到歌单" aria-label="收藏到歌单" data-stop-propagation="true" data-action="collectArtistDetailSong" data-action-value="' + i + '">' + artistCollectTrayIconSvg() + '</button>' +
      '<button class="artist-song-action next" type="button" title="下一首播放" aria-label="下一首播放" data-stop-propagation="true" data-action="queueArtistDetailSongNext" data-action-value="' + i + '">' + artistNextPlusIconSvg() + '</button>' +
    '</div>';
    return '<div class="artist-song-item" data-action="playArtistDetailSong" data-action-value="' + i + '">' +
...
function artistHomePlayIconSvg() {
  return '<svg class="artist-home-play-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M8 5.5v13l10-6.5-10-6.5z"/></svg>';
}
function renderNavidromeArtistHome(data) {
  detailArtistMeta = (data && data.artist) ? Object.assign({}, data.artist) : null;
  detailArtistSongs = [];
  detailArtistRandomSongs = [];
  detailArtistAlbums = ((data && data.albums) || []).map(function(album){ return Object.assign({}, album); });
  var albumCount = detailArtistAlbums.length;
  var albumsHtml = albumCount ? ('<div class="artist-album-grid">' + detailArtistAlbums.map(function(album, i){
    var cover = album.cover ? coverUrlWithSize(album.cover, 150) : '';
    var coverHtml = cover ? '<img class="artist-album-cover" src="' + escHtml(cover) + '" alt="" onerror="this.style.opacity=0.18">' : '<div class="artist-album-cover"></div>';
    var meta = [album.year || '', album.trackCount ? (album.trackCount + ' 首') : ''].filter(Boolean).join(' · ');
    return '<button class="artist-album-card" type="button" data-action="openNavidromeArtistAlbum" data-action-value="' + i + '">' +
      coverHtml +
      '<span class="artist-album-name">' + escHtml(album.name || '未知专辑') + '</span>' +
      '<span class="artist-album-meta">' + escHtml(meta || 'Navidrome 专辑') + '</span>' +
    '</button>';
  }).join('') + '</div>') : '<div class="detail-empty">暂无专辑</div>';
  return '<div class="artist-home-actions">' +
      '<button class="artist-home-btn primary" type="button" data-action="playNavidromeArtistTopSongs">' + artistHomePlayIconSvg() + '<span>热门歌曲</span></button>' +
      '<button class="artist-home-btn" type="button" data-action="playNavidromeArtistRandomSongs">' + artistHomePlayIconSvg() + '<span>随机播放</span></button>' +
      '<span class="artist-home-count">' + albumCount + ' 张专辑</span>' +
    '</div>' +
    '<div class="detail-section"><div class="detail-section-head"><div class="detail-section-title">全部专辑</div></div>' + albumsHtml + '</div>';
}
function loadNavidromeArtistSongs(mode) {
  var meta = detailArtistMeta || {};
  var params = [];
  if (meta.id) params.push('id=' + encodeURIComponent(meta.id));
  if (meta.name) params.push('name=' + encodeURIComponent(meta.name));
  params.push('mode=' + encodeURIComponent(mode === 'random' ? 'random' : 'top'));
  params.push('limit=50');
  if (!meta.id && !meta.name) return Promise.resolve([]);
  showToast(mode === 'random' ? '正在生成随机队列...' : '正在载入热门歌曲...');
  return apiJson('/api/navidrome/artist/songs?' + params.join('&')).then(function(r){
    if (r && r.error) throw new Error(r.error);
    return ((r && r.songs) || []).map(cloneSong);
  });
}
function playDetailArtistQueue(list, label) {
  var songs = (list || []).map(cloneSong).filter(function(song){ return song && song.id; });
  if (!songs.length) { showToast('暂无可播放歌曲'); return; }
  playQueue = songs;
  currentIdx = 0;
  safeRenderQueuePanel('artist-detail-queue');
  safeSwitchPlaylistTab('queue', 'artist-detail-queue');
  safeShelfRebuild('artist-detail-queue', true);
  closeTrackDetailModal();
  showToast(label || '已加入播放队列');
  playQueueAt(0).catch(function(e){ console.warn('[ArtistDetailQueuePlay]', e); });
}
function playNavidromeArtistTopSongs() {
  loadNavidromeArtistSongs('top').then(function(songs){
    detailArtistSongs = songs;
    playDetailArtistQueue(songs, '已按服务器热门歌曲播放');
  }).catch(function(e){
    console.warn('[NavidromeArtistTopSongs]', e);
    showToast('热门歌曲加载失败');
  });
}
function playNavidromeArtistRandomSongs() {
  loadNavidromeArtistSongs('random').then(function(songs){
    detailArtistRandomSongs = songs;
    playDetailArtistQueue(songs, '已随机播放该歌手歌曲');
  }).catch(function(e){
    console.warn('[NavidromeArtistRandomSongs]', e);
    showToast('随机歌曲加载失败');
  });
}
...
function detailNextIconSvg() {
  return '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 6h4l6 6-6 6H5l6-6-6-6z"/><path d="M19 5v14"/></svg>';
}
function detailQueueIconSvg() {
  return '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 6h11"/><path d="M4 11h11"/><path d="M4 16h7"/><path d="M18 13v7"/><path d="M14.5 16.5h7"/></svg>';
}
function detailDownloadIconSvg() {
  return '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 4v10"/><path d="M7.5 9.5 12 14l4.5-4.5"/><path d="M5 18.5h14"/></svg>';
}
function detailPlusIconSvg() {
  return '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 5.5v13"/><path d="M5.5 12h13"/></svg>';
}
function detailFormatBytes(bytes) {
...
function renderNavidromeAlbumDetail(album, tracks, loading) {
  detailAlbumMeta = album ? Object.assign({}, album) : null;
  detailAlbumSongs = (tracks || []).map(cloneSong);
  var cover = album && album.cover ? coverUrlWithSize(album.cover, 240) : '';
  var totalBytes = detailAlbumSongs.reduce(function(sum, song){ return sum + (Number(song.size) || 0); }, 0);
  var totalDuration = detailAlbumSongs.reduce(function(sum, song){ return sum + (Number(playbackDurationFromSong(song)) || 0); }, 0);
  var coverHtml = cover ? '<img class="detail-cover album-detail-cover" src="' + escHtml(cover) + '" alt="" onerror="this.style.opacity=0.18">' : '<div class="detail-cover album-detail-cover"></div>';
  var metaBits = [
    album && album.year ? album.year : '',
    detailAlbumSongs.length ? (detailAlbumSongs.length + ' 首') : (album && album.trackCount ? album.trackCount + ' 首' : ''),
    totalDuration ? formatProgramTime(totalDuration) : '',
    totalBytes ? detailFormatBytes(totalBytes) : ''
  ].filter(Boolean).join(' · ');
  var actions = '<div class="album-detail-actions">' +
    '<button class="album-detail-action primary" type="button" data-action="playNavidromeAlbumNow">' + detailPlayIconSvg() + '<span>立即播放</span></button>' +
    '<button class="album-detail-action" type="button" data-action="shuffleNavidromeAlbum">' + detailShuffleIconSvg() + '<span>随机播放</span></button>' +
    '<button class="album-detail-action" type="button" data-action="queueNavidromeAlbumNext">' + detailNextIconSvg() + '<span>下一首播放</span></button>' +
    '<button class="album-detail-action" type="button" data-action="appendNavidromeAlbumToQueue">' + detailQueueIconSvg() + '<span>添加到播放队列</span></button>' +
    '<button class="album-detail-action" type="button" data-action="collectNavidromeAlbum">' + playlistPlusIconSvg() + '<span>添加到歌单</span></button>' +
    '<button class="album-detail-action" type="button" data-action="downloadNavidromeAlbum">' + detailDownloadIconSvg() + '<span>下载' + (totalBytes ? (' ' + detailFormatBytes(totalBytes)) : '') + '</span></button>' +
  '</div>';
  var rows = loading
    ? '<div class="detail-loading">正在载入专辑歌曲...</div>'
    : detailAlbumSongs.map(function(song, i){
      if (song && song.id && song.starred) likedSongMap[String(song.id)] = true;
      var suffix = song.suffix ? String(song.suffix).toUpperCase().replace(/^AUDIO\//, '') : '';
      return '<div class="album-song-row" data-action="playNavidromeAlbumTrack" data-action-value="' + i + '">' +
        '<div class="album-song-rank">' + String(i + 1).padStart(2, '0') + '</div>' +
        '<div class="album-song-main"><div class="album-song-title">' + escHtml(song.name || '') + '</div>' +
        '<div class="album-song-meta">' + escHtml((song.artist || '未知歌手') + (songDurationLabel(song) ? (' · ' + songDurationLabel(song)) : '') + (suffix ? (' · ' + suffix) : '')) + '</div></div>' +
  ...
  if (!loading && !rows) rows = '<div class="detail-empty">专辑暂无歌曲</div>';
  return '<div class="detail-hero album-detail-hero">' + coverHtml +
      '<div style="min-width:0;flex:1"><div class="detail-title">' + escHtml(album && album.name || 'Navidrome 专辑') + '</div>' +
      '<div class="detail-sub">' + escHtml((album && album.creator) || (detailAlbumSongs[0] && detailAlbumSongs[0].artist) || 'Navidrome') + '</div>' +
      '<div class="album-detail-meta">' + escHtml(metaBits || 'Navidrome 专辑') + '</div></div>' +
    '</div>' +
    actions +
    '<div class="detail-section album-detail-section"><div class="detail-section-head"><div class="detail-section-title">全部歌曲</div></div><div class="album-song-list">' + rows + '</div></div>';
}
function openNavidromeAlbumDetail(album) {
  if (!album || !album.id) return;
  var heading = document.getElementById('track-detail-heading');
  var body = document.getElementById('track-detail-body');
  ...
  body.innerHTML = renderNavidromeAlbumDetail(album, [], true);
  bindTrackDetailScrollers();
  openGsapModal(document.getElementById('track-detail-modal'));
  apiJson('/api/navidrome/playlist/tracks?id=' + encodeURIComponent(album.id) + '&limit=500').then(function(r){
    if (seq !== trackDetailSeq) return;
    var nextAlbum = r && r.playlist ? Object.assign({}, album, r.playlist) : album;
    body.innerHTML = renderNavidromeAlbumDetail(nextAlbum, (r && r.tracks) || [], false);
    bindTrackDetailScrollers();
  }).catch(function(e){
    console.warn('[NavidromeAlbumDetail]', e);
    if (seq === trackDetailSeq) body.innerHTML = '<div class="detail-empty">专辑详情加载失败</div>';
  });
}
function playNavidromeAlbumTrack(i) {
...
function playNavidromeAlbumNow() {
  if (!detailAlbumSongs.length) { showToast('专辑暂无可播放歌曲'); return; }
  playNavidromeAlbumTrack(0);
}
function shuffleNavidromeAlbum() {
  var songs = detailAlbumSongs.map(cloneSong);
...
function insertSongsNext(songs) {
  songs = (songs || []).map(cloneSong).filter(Boolean);
  for (var i = songs.length - 1; i >= 0; i--) queueSongNext(songs[i]);
}
function queueNavidromeAlbumNext() {
  insertSongsNext(detailAlbumSongs);
  showToast('已设为接下来播放');
}
function appendNavidromeAlbumToQueue() {
  detailAlbumSongs.forEach(function(song){ queueSong(song); });
  showToast('已添加到播放队列');
}
...
function navidromeDownloadFile(song) {
  ...
  var ext = song.suffix ? ('.' + String(song.suffix).replace(/^\./, '').replace(/[^\w]+/g, '')) : '';
  a.href = '/api/navidrome/download?id=' + encodeURIComponent(song.id);
  a.download = ((song.artist ? song.artist + ' - ' : '') + (song.name || 'Navidrome') + ext).replace(/[\\/:*?"<>|]+/g, ' ');
  document.body.appendChild(a);
  a.click();
  setTimeout(function(){ try { document.body.removeChild(a); } catch (e) {} }, 0);
}
function downloadNavidromeAlbum() {
  if (!detailAlbumSongs.length) { showToast('专辑暂无可下载歌曲'); return; }
  detailAlbumSongs.slice(0, 80).forEach(function(song, i){
    setTimeout(function(){ navidromeDownloadFile(song); }, i * 180);
  });
  showToast('已开始下载专辑歌曲');
}
function downloadNavidromeAlbumTrack(i) {
  navidromeDownloadFile(detailAlbumSongs[i]);
}
...
function openTrackDetailModal(type, songOverride) {
  ...
  if (type === 'artist') {
    ...
    var isNavidromeArtist = songProviderKey(song) === 'navidrome';
    var artistName = artists.join(' / ') || song.artist || '未知歌手';
    var artistDetailUrl = isNavidromeArtist
      ? ('/api/navidrome/artist/detail?id=' + encodeURIComponent(artistId || '') + '&name=' + encodeURIComponent(artistName === '未知歌手' ? '' : artistName) + '&limit=50')
      : (artistId
    ...
    body.innerHTML =
      ...
      '<div id="artist-home-root">' + (isNavidromeArtist
        ? (artistDetailUrl ? '<div class="detail-section"><div class="detail-loading">' + escHtml(artistLoadingText) + '</div></div>' : '<div class="detail-section"><div class="detail-empty">' + escHtml(artistEmptyText) + '</div></div>')
        : ('<div class="detail-section"><div class="detail-section-head"><div class="detail-section-title">热门歌曲</div></div><div id="artist-hot-songs">' + (artistDetailUrl ? '<div class="detail-loading">' + escHtml(artistLoadingText) + '</div>' : '<div class="detail-empty">' + escHtml(artistEmptyText) + '</div>') + '</div></div>')) + '</div>';
    if (artistDetailUrl) {
      apiJson(artistDetailUrl).then(function(r){
        if (seq !== trackDetailSeq) return;
        var returnedName = r && r.artist && r.artist.name;
        var target = isNavidromeArtist ? document.getElementById('artist-home-root') : document.getElementById('artist-hot-songs');
        ...
        if (r && r.artist && r.artist.avatar) {
          ...
          if (avatarEl) {
            ...
            avatarEl.style.backgroundImage = 'url("' + cssImageUrl(coverUrlWithSize(r.artist.avatar, 180)) + '")';
            avatarEl.style.backgroundSize = 'cover';
            avatarEl.style.backgroundPosition = 'center';
          }
        }
        if (target) target.innerHTML = r && !r.error ? (isNavidromeArtist ? renderNavidromeArtistHome(r) : renderArtistSongList(r.songs || [])) : '<div class="detail-empty">歌手主页加载失败</div>';
        bindTrackDetailScrollers();
      }).catch(function(){
        var target = isNavidromeArtist ? document.getElementById('artist-home-root') : document.getElementById('artist-hot-songs');
...
function isSongLiked(song) {
  return !!(song && song.id && (likedSongMap[String(song.id)] || song.starred));
}
...
function songActionHtml(kind, source, index, song) {
  var liked = isSongLiked(song);
  var likeAction = source === 'SearchResult' ? 'toggleLikeSearchResult'
    : source === 'QueueIndex' ? 'toggleLikeQueueIndex'
    : source === 'ArtistDetailSong' ? 'toggleLikeArtistDetailSong'
    : '';
  var collectAction = source === 'SearchResult' ? 'collectSearchResult'
    : source === 'QueueIndex' ? 'collectQueueIndex'
    : source === 'ArtistDetailSong' ? 'collectArtistDetailSong'
    : '';
  if (kind === 'like') {
    return '<button class="song-action-btn' + (liked ? ' liked' : '') + '" title="' + (liked ? '取消红心' : '红心喜欢') + '" data-stop-propagation="true" data-action="' + likeAction + '" data-action-value="' + index + '">' + heartIconSvg() + '</button>';
  }
  return '<button class="song-action-btn" title="收藏到歌单" data-stop-propagation="true" data-action="' + collectAction + '" data-action-value="' + index + '">' + playlistPlusIconSvg() + '</button>';
}
...
function markSongsLiked(songs, liked) {
  (songs || []).forEach(function(song){
    if (isCloudSong(song) || songProviderKey(song) === 'navidrome') likedSongMap[String(song.id)] = !!liked;
  });
}
function refreshNavidromeAlbumLikeStates() {
  var body = document.getElementById('track-detail-body');
  if (!body || !detailAlbumSongs || !detailAlbumSongs.length) return;
  Array.prototype.forEach.call(body.querySelectorAll('[data-action="toggleLikeNavidromeAlbumTrack"]'), function(btn){
    var i = Number(btn.getAttribute('data-action-value'));
...
async function toggleLikeSong(song) {
  if (songProviderKey(song) === 'navidrome') {
    if (!navidromeStatus.loggedIn) {
      showToast('先连接 Navidrome');
      showLoginModal();
      return;
    }
    var navId = String(song && song.id || '');
    if (!navId) return;
    var busyKey = 'navidrome:' + navId;
    if (likeBusyMap[busyKey]) return;
    var navNext = !isSongLiked(song);
    likeBusyMap[busyKey] = true;
    likedSongMap[navId] = navNext;
    song.starred = navNext;
    updateLikeButtons(song);
    safeRenderQueuePanel('navidrome-like-toggle', { scrollCurrent: miniQueueOpen });
    refreshSearchResultActionStates();
    refreshNavidromeAlbumLikeStates();
    try {
      var nr = await apiJson('/api/navidrome/star?id=' + encodeURIComponent(navId) + '&starred=' + encodeURIComponent(String(navNext)));
      if (nr && nr.error) throw new Error(nr.error);
      likedSongMap[navId] = navNext;
      song.starred = navNext;
      showToast(navNext ? '已加入 Navidrome 收藏' : '已取消 Navidrome 收藏');
    } catch (err) {
      likedSongMap[navId] = !navNext;
      song.starred = !navNext;
      showToast('Navidrome 红心操作失败');
    } finally {
      delete likeBusyMap[busyKey];
      updateLikeButtons(song);
      safeRenderQueuePanel('navidrome-like-final', { scrollCurrent: miniQueueOpen });
      refreshSearchResultActionStates();
      refreshNavidromeAlbumLikeStates();
    }
    return;
  ...
  if (!isCloudSong(song)) {
    ...
    showToast(key === 'qq' ? 'QQ 音乐红心同步待登录接口接入' : '本地文件暂不支持红心同步');
    return;
  }
