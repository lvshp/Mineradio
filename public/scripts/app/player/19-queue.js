...
function playbackProviderLabel(song) {
  ...
  if (key === 'navidrome') return 'Navidrome';
...
function alternatePlaybackProvider(song) {
  if (songProviderKey(song) === 'navidrome') return '';
...
async function tryAutoPlaybackFallback(song, data, idx, token, opts) {
  ...
  if (!song || song.type === 'local' || song.type === 'podcast' || song.source === 'podcast' || songProviderKey(song) === 'navidrome') return false;
...
async function playQueueAt(idx, opts) {
  ...
  try {
    ...
    var data = isNavidromePlayback
      ? await apiJson('/api/navidrome/song/url?id=' + encodeURIComponent(song.id || ''))
      : (isQQPlayback
    ...
    if (!isQQPlayback && !isNavidromePlayback && playbackQualityWasDowngraded(requestedQuality, data.level)) {
      showSourceFallbackNotice('网易云音质自动降级', '请求 ' + playbackQualityLabel(requestedQuality) + '，实际播放 ' + resolvedQualityText + '。');
    } else if (opts.qualitySwitch && !isNavidromePlayback) {
    ...
    if (data.trial) {
      ...
      if (trialLoginBtn) {
        ...
        trialLoginBtn.setAttribute('data-action', 'openProviderLogin');
        trialLoginBtn.setAttribute('data-action-value', 'netease');
      }
    ...
    applyVolumeToAudio();
    var proxyAudioUrl = data.proxy || /^\//.test(String(data.url || '')) ? data.url : '/api/audio?url=' + encodeURIComponent(data.url);
