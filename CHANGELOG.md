...
- Navidrome 歌单体验补齐：真实歌单进入“我的歌单”，全部专辑单独进入登录后显示的“ND专辑”，避免歌单页被全部专辑占满。
- Navidrome 收藏到歌单实装：ND 歌曲收藏时弹出 ND 歌单选择，添加前检测重复，已存在的歌曲跳过并给出提示。
- 专辑详情和歌手主页操作补齐：专辑/歌曲列表增加喜欢、下一个播放、添加到队列、添加到歌单等按钮；从歌手主页进入专辑详情后关闭会返回歌手主页。
- 媒体缓存升级：Navidrome 和第三方歌词、图片永久缓存到本地，单曲缺图时会缓存第三方封面，下次优先走缓存；账号信息里新增清理缓存按钮并显示缓存大小。
- 修复歌词缓存空结果问题：歌词接口只缓存真正拿到的歌词，旧的空歌词缓存会被忽略并重新拉取。
- 歌手头像增加外部兜底：Navidrome 没有歌手图时尝试拉取第三方歌手图片，再回落专辑封面或默认占位。
- 记录首页左侧下一步方案：左侧改为沉浸播放看板，并融合私人电台快捷入口；右侧继续承载歌单、专辑和推荐内容。
