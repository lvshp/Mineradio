...
- 当前可运行程序：以当前构建产物或安装位置为准，不在项目规则里写死机器路径
- 当前代码/Git 仓库：项目根目录，也就是包含 `AGENTS.md`、`package.json`、`server.js` 的目录
- GitHub 仓库：以当前 `git remote -v` 输出为准
- 当前源码版本：以 `package.json` 的 `version` 为准
- 统一备份区：项目根目录外的备份目录，不要写死机器路径

## Start Every New Codex Thread Here

新对话开始处理 Mineradio 前，必须先确认当前目录是项目根目录：
...
前端主逻辑已拆分到 `public/index.html`、`public/styles/main.css` 和 `public/scripts/app/**`。改前端源码后必须运行 `npm run build:app` 更新 `public/scripts/app.bundle.js`。没有独立 npm test，改动后至少做：

注意：本地依赖以项目根目录的 `node_modules` 为准。如果发布打包时缺少 `electron-builder`，先在项目根目录执行 `npm install`，再执行 `npm run build:win`。
...
- 备份策略：不要删除旧资料；重复和历史内容移动到项目根目录外的备份区。
- 重要：所有源码改动都以项目根目录下的 `public/`、`desktop/`、`server.js` 等为准，不要再按旧机器绝对路径寻找源码。
